...
const get_bin_manifest = async ({setFeedback}:any)=>{
    if (import.meta.env.DEV && import.meta.env.VITE_DEV_USE_LOCAL_BIN_MANIFEST === "1") {
        try{
            return {code:200, data:JSON.parse(await readTextFile(import.meta.env.VITE_DEV_LOCAL_BIN_MANIFEST_PATH))};
        }catch(e){
        ...
        if (manifest_response.code === 200) {
            ...
            setFeedback({text:`Failed to check bin manifest.`, color:"red",type:"error"})
            return {code:500, message:manifest_response?.message};
        }
    }
}

export default get_bin_manifest;