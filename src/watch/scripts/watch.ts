...
export function get_episode_server(source: string, pluginId: string, seasonIndex: number, episodeIndex: number, episodeId: string): Promise<EpisodeServerData> {
    return invoke<EpisodeServerData>('get_episode_server', {source, pluginId, seasonIndex, episodeIndex, episodeId});
}
...
export function update_watch_index(source: string, id: string, seasonIndex: number, episodeIndex: number): Promise<void> {
    return invoke('update_watch_index', {source, id, seasonIndex, episodeIndex});
}
